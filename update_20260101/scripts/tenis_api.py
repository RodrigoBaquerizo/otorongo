...
### HELPER FUNCTIONS ###
def save_to_csv_simple(data: requests.Response, filename: str) -> None:
...
def get_fixtures(
    ...
    logging.info(f"Saved {len(df_events)} event records for player {player_key}")
    ...
    if pointbypoint_records:
        ...
        logging.info(
            f"Saved {len(df_pbp)} pointbypoint records for player {player_key}"
        )

    if scores_records:
        df_scores = pd.DataFrame(scores_records)
        df_scores.to_csv(f"{file_path}_scores.csv", index=False, encoding="utf-8")
        logging.info(f"Saved {len(df_scores)} scores records for player {player_key}")
...
def get_standings(
    ...
    save_to_csv_simple(data, f"data/standings_{event_type}.csv")
